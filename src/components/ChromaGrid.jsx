...
const ChromaGrid = ({
  ...
  fadeOut = 0.6,
  ease = "power3.out",
  imageSize, // "xs" | "sm" | "md" | "lg" | "xl" — default do grid, opcional
}) => {
  ...
  return (
    ...
    >
      {data.map((c, i) => {
        // tamanho vem do item, senão do grid, senão fica no comportamento antigo (imagem esticada)
        const size = c.imageSize || imageSize;
        const imgSizeClass = size ? IMAGE_SIZES[size] : "w-full h-full";
